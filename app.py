    ...
    if fixture_id:
        try:
            ...
            for event in fetch_api_football_fixture_events(API_FOOTBALL_TOKEN, fixture_id):
                ...
                if is_opponent and not is_shootout and detail == "penalty":
...
<div class='payout-desc'><b>Data APIs</b><br>
The app uses two soccer data APIs. <b>Football-Data.org</b> powers the normal World Cup match feed: fixtures, scores, goal events, assists, team match points, player points, and advancement updates. <b>API-Football by API-SPORTS</b> powers the separate Goalie Challenge: World Cup fixtures for goalie rounds, goalkeeper squad/photo data, goalkeeper saves, penalty saves when available, goals conceded, and goalie live scoring. API tokens are configured only in Streamlit Cloud secrets as <code>FOOTBALL_DATA.TOKEN</code> and <code>API_FOOTBALL.TOKEN</code>.</div>
